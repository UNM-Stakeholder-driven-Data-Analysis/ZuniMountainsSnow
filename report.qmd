...
![TODO: replace this with one zoomed in optimal image for each criteria, for one chosen year](./GeneratedPlots/zoom_sim_opt_compare.jpeg){#fig-zoom}
...
![The random points used to measure canopy diameter in Google Earth Pro. The measurement for each point was the maximum width of the nearest discernible tree-shadow. Imagery Date: 3/25/2015](./diagrams/randomPoints.jpg){#fig-randomPoints}

![An example of a measurement being made on a discernible tree-shadow](./diagrams/shadowCluster.jpg){#fig-shadowCluster}

![Tree crown diameters for the 20 randomly generated points within the D2-BW thinning polygon, for Google Earth Pro imagery, 3/25/2015](./GeneratedPlots/crownDiams.jpeg){#fig-crownDiams}

### Visual Assessment of Snow Accumulation Patterns {#sec-visAsses}

### Comparing to Real Imagery

The following figures depict Google earth imagery next to one stochastic simulation for the same year or nearby dates. Note that visual comparisons at this scale are biased by the presence of shadows in the real image. Shadows make canopy cover appear more dense than it really is. However, these comparisons can still be used to get a rough idea of how well the simulations approximate reality across different years. The nearest image available on Google Earth to the year 2000 is 1997, so it is omitted. Google Earth did not have an image for the year 2010, but did have images for 2009 and 2011. Both are shown below (@fig-2009-google and @fig-2011-google). The years 2005 and 2010 had higher average percent error than 2015 (see \@fig-hist), which may explain the presence of spurious low tree cover areas which appear in @fig-2009-google and @fig-2011-google. Overall the simulations do capture large scale (\>30m) patterns in tree cover density. This is highlighted in @fig-2015-google, where arrows indicate regions of high and low density that correspond in both images, respectively. It is notable that simulated images result in more small patches of tree cover distributed in low density areas, whereas real imagery indicates larger clearings between tree cover (see blue arrow in @fig-2015-google). This difference can be diminished by the choice of optimal ground criteria. For example, the North Line criteria (see @sec-north-line) with minimum cluster size of 2 effectively ignores single isolated cells in a clearing. #TODO add the optimal image to compare. this might be a good idea because even though trees exist in a clearing they may be discounted by a criteria.

![Comparison between Google Earth Imagery (date: June 30th, 2005) and a simulation of tree cover for 2005](./diagrams/2005_compare_sim.jpg){#fig-2005-google}

![Comparison between Google Earth Imagery (date: May 8th, 2009) and a simulation of tree cover for 2010](./diagrams/2010_compare_sim_2009.jpg){#fig-2009-google}

![Comparison between Google Earth Imagery (date: July 7th, 2011) and a simulation of tree cover for 2010](./diagrams/2010_compare_sim_2011.jpg){#fig-2011-google}

![Comparison between Google Earth Imagery (date: March 25th, 2015) and a simulation of tree cover for 2015. Magenta arrows indicate a region of high density tree cover that corresponds in both images. Blue arrows indicate a region of low density tree cover that corresponds in both images.](./diagrams/2015_compare_sim.jpg){#fig-2015-google}
