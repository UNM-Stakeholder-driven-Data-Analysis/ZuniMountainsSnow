...
![An example of a random simulation of trees 30x30 meter cell with 30% tree cover. 0 Represents bare ground, 100 represents tree-cover.](./GeneratedPlots/one30x30randSim.jpeg){#fig-randExample fig-align="center" width="400"}
...
All criteria were applied to each of the 100 simulated images and across all years. A zoomed in image of each criterion applied to one stochastic simulation for the year 2015 is shown in @fig-zoom. A comprehensive set of optimal .

![Zoomed in results of each criteria applied to one simulation, for the year 2015. Grey lines indicate 30mx30m cells. North Semicircle criteria: the number indicates the minimum number of cells to qualify the area as optimal. North Line criteria: the number indicates the minimum cluster size required to qualify an area as optimal. TODO: remove axis labels, change figure title](./GeneratedPlots/optAreaZoom_2015.jpeg){#fig-zoom}
...
![Total Optimal Area for all simulations. Each simulation's optimal area is represented by a dot with a different horizontal position. All the dots cluster very tightly relative to the variation between years, making the vertical difference between dots for one year nearly imperceptible. Lines are shown to help visualize the relative magnitude of change between criteria.](./GeneratedPlots/totOpt_all.jpeg){#fig-totOptArea}
...
The simplest model based on visual evidence from Google Earth Pro represents snowpack as occurring on the north side of tree cover (North Line criteria) (see @sec-visAsses, North Line data). Both the generous (minimum cluster size set to 1 cell) and more conservative (minimum cluster size set to 2 cells) versions of this criteria result in a reduction in optimal area after thinning.
...
The cell size for the stochastic simulation needed to be smaller than 15 meters, but not too small. Cells that were too small would result in isolated patches of tree cover smaller than those found in reality, and would result in unnecessarily slow computations. For this reason Google Earth Pro Pro imagery from 2015 was used to find a representative lower limit for tree canopy diameter. The image from 2015 had lower tree density, making it easier to measure canopy diameter. Twenty points were randomly distributed within the D2-BW polygon. These were exported to Google Earth Pro Pro (see @fig-randomPoints). In Google Earth Pro the nearest discernible tree shadow to each point was identified. Clusters of multiple trees who's shadows could not be distinguished into separate trees were discounted (see @fig-shadowCluster for an example). If high tree density resulted in no discernible shadow in the neighborhood of the point, the canopy itself was measured. The shadow measuremet was preffered because it provides a higher contrast image of the tree canopy against the ground. The sun's rays are assumed to be approximately parallel. Therefore the shadow's maximum width represents the tree's width from one particular vantage point. The distribution of resulting crown diameters are visualized in @fig-crownDiams.

@fig-crownDiams indicates that a reasonable lower limit for crown diameters is 3 meters. The density plot of the distributions shows a bi-modal distribution, with a lower peak located slightly less than 3meters, and an upper peak close to 4.5 meters. Because the stochastic simulation simulates tree cover, and not necessarily tree presence/absence directly, it was decided to use the lower limit. Clusters of more than one covered 3mx3m cell can represent larger trees. Although the lower peak of the distribution is closer to 3.8 meters, 3 meters was chosen because it is an even divisor of 30, and therefore avoids rounding error. This choice ensures that stochastic simulations will exactly preserve the percent cover values at the 30x30 meter scale.
...
![Tree crown diameters for the 20 randomly generated points within the D2-BW thinning polygon, for Google Earth Pro Pro imagery, 3/25/2015. Each crown diameter value is represented as a point with a slight random offset in the horizontal direction, making it easier to see where points overlap. The violin plot is also displayed.](./GeneratedPlots/crownDiams.jpeg){#fig-crownDiams width="2.88in"}
